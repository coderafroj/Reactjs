...
function App() {
  ...
  //useref==password referace and use ref atriubte
...
const copyBtn=useCallback(()=>{
  ...
  window.navigator.clipboard.writeText(password)
  passRef.current.select(password)//copy krn epr high lifht ho jayega
  passRef.current.setSelectionRange(0,3)//isse apne hisaas se letter copy kr skte hai  jitne krna ho
    ...
    <div className='w-full flex justify-center rounded-3xl py-6 px-3 shadow-2xs items-center'>
      ...
      <div className='max-w-md bg-red-400 py-6 px-6 rounded-4xl'>
        ...
        <div className='mx-w-md w-full'>
          <input type="text" className='bg-white rounded-l-lg px-2 py-1 w-[80%] outline-none 'ref={passRef} readOnly value={password} />
